    ...
    for attempt in range(sess.max_retries + 1):
        ...
        try:
            with requests.post(url, headers=headers, json=payload, stream=sess.stream,
                ...
                if r.status_code >= 400:
                    if r.status_code in _RETRYABLE and attempt < sess.max_retries:
                        d = _delay(r, attempt)
                        logger.debug(f"[LLM Retry] HTTP {r.status_code}, retry in {d:.1f}s ({attempt+1}/{sess.max_retries+1})")
                        time.sleep(d)
                        continue
                    try:
            ...
            if attempt < sess.max_retries:
                ...
                logger.debug(f"[LLM Retry] {type(e).__name__}, retry in {d:.1f}s ({attempt+1}/{sess.max_retries+1})")
                yield err
                time.sleep(d)
                continue
            yield err
    ...
    def _apply_claude_thinking(self, payload):
        ...
        if self.reasoning_effort:
            ...
            if effort:
                ...
                logger.warning(f"[WARN] reasoning_effort {self.reasoning_effort!r} is unsupported for Claude output_config.effort, ignored.")
    def ask(self, prompt):
        def _ask_gen():
            with self.lock:
...
class ClaudeSession(BaseSession):
    def raw_ask(self, messages):
        ...
        headers = {"x-api-key": self.api_key, "Content-Type": "application/json", "anthropic-version": ANTHROPIC_VERSION, "anthropic-beta": ANTHROPIC_BETA}
    ...
    def _raw_ask(self, *args, **kwargs):
        ...
        for attempt in range(self._retries + 1):
            ...
            if not is_err:
                ...
                self._switched_at = time.time()
                logger.debug(f'[MixinSession] Partial failure, next call → s{self._cur_idx} ({self._sessions[self._cur_idx].name})')
                return return_val
