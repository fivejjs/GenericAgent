...
class Session:
    ...
    def mark_disconnected(self):
        if self.is_active():
            logger.info(f"Tab disconnected: {self.url} (Session: {self.id})")
...
class TMWebDriver:
    ...
    def start_ws_server(self) -> None:
        ...
        class JSExecutor(WebSocket):
            ...
            def connected(self):
                (f"New connection from {self.address}")
    ...
    def _remote_cmd(self, cmd):
        try:
            return requests.post(self.remote, headers={"Content-Type": "application/json"}, json=cmd, timeout=30).json()
    ...
    def set_session(self, url_pattern: str) -> bool:
        ...
        if not matched:
            return logger.info(f"警告: 未找到URL包含 '{url_pattern}' 的会话")
        if len(matched) > 1:
            logger.info(f"警告: 找到多个URL包含 '{url_pattern}' 的会话，选择第一个")
