...
def load_tool_schema(suffix=""):
    global TOOLS_SCHEMA
    with open(
        os.path.join(script_dir, f"assets/tools_schema{suffix}.json"),
        "r",
        encoding="utf-8",
    ) as f:
        TS = f.read()
...
def get_system_prompt():
    with open(
        os.path.join(script_dir, f"assets/sys_prompt{lang_suffix}.txt"),
        "r",
        encoding="utf-8",
    ) as f:
        prompt = f.read()
...
class GenericAgent:
    def __init__(self):
        ...
        self.log_path = os.path.join(
            script_dir,
            f"temp/model_responses/model_responses_{int(time.time() * 1e6) % 1000000:06d}.txt",
        )
    ...
    def run(self):
        while True:
            ...
            raw_query, source, display_queue = (
                task["query"],
                task["source"],
                task["output"],
            )
            ...
            try:
                ...
                logger.info(f"Backend Error: {format_error(e)}")
                display_queue.put(
                    {
                        "done": full_resp + f"\n```\n{format_error(e)}\n```",
                        "source": source,
...
GeneraticAgent = GenericAgent  # legacy alias — prefer GenericAgent in new code
        ...
        while True:
            if os.path.getmtime(args.reflect) != _mt:
                try:
                    ...
                    logger.info(f"[Reflect] reload error: {e}")
            time.sleep(getattr(mod, "INTERVAL", 5))
            try:
                task = mod.check()
            except Exception as e:
                logger.info(f"[Reflect] check() error: {e}")
                continue
            if task and task == "/exit":
                break
            if task is None:
                continue
            logger.info(f"[Reflect] triggered: {task[:80]}")
            ...
            try:
                ...
                if getattr(mod, "ONCE", False):
                    raise
                logger.info(f"[Reflect] drain error: {e}")
                result = f"[ERROR] {e}"
            ...
            if on_done := getattr(mod, "on_done", None):
                try:
                    ...
                    logger.info(f"[Reflect] on_done error: {e}")
            if getattr(mod, "ONCE", False):
        ...
        while True:
            ...
            try:
                ...
                while True:
                    item = dq.get()
                    if "next" in item:
                        logger.info(item["next"], end="", flush=True)
                    if "done" in item:
                        logger.info()
                        break
