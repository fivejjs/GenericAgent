...
from agent_loop import BaseHandler, StepOutcome, json_default  # noqa: E402
...
def file_read(path, start=1, keyword=None, count=200, show_linenos=True):
    try:
        with open(path, "r", encoding="utf-8", errors="replace") as f:
            ...
            res = [(i, line_text if len(line_text) <= L_MAX else line_text[:L_MAX] + TAG) for i, line_text in res]
            result = "\n".join(f"{i}|{line_text}" if show_linenos else line_text for i, line_textine_text in res)
            if show_linenos:
...
class GenericAgentHandler(BaseHandler):
    ...
    def _extract_code_block(self, response, code_type):
        code_type = {"python": "python|py", "powershell": "powershell|ps1|pwsh", "bash": "bash|sh|shell"}.get(
    ...
    def do_code_run(self, args, response):
        ...
        result = yield from code_run(
            code, code_type, timeout, cwd, code_cwd=code_cwd, stop_signal=self.code_stop_signal, maxlen=maxlen
        )
    ...
    def do_web_execute_js(self, args, response):
        ...
        if not script:
            return StepOutcome("[Error] Script missing. Use ```javascript block or 'script' arg.", next_prompt="\n")
        ...
        try:
            logger.info("Web Execute JS Result:", show)
        except Exception:
            pass
        yield f"JS 执行结果:\n{show}\n"
        next_prompt = self._get_anchor_prompt(skip=args.get("_index", 0) > 0)
    ...
    def enter_plan_mode(self, plan_path):
        ...
        self.max_turns = 100
        logger.info(f"[Info] Entered plan mode with plan file: {plan_path}")
        return plan_path

    def _check_plan_completion(self):
        if not os.path.isfile(p := self._in_plan_mode() or ""):
            return None
        try:
...
""" + get_global_memory()
    ...
    def turn_end_callback(self, response, tool_calls, tool_results, turn, next_prompt, exit_reason):
        ...
        if rsumm:
            ...
            tc = tool_calls[0]
            tool_name, args = tc["tool_name"], tc["args"]  # at least one because no_tool
        ...
        if _plan and turn >= 10 and turn % 5 == 0:
            next_prompt = (
                f"[Plan Hint] 正在计划模式。必须 file_read({_plan}) 确认当前步骤，回复开头引用：📌 当前步骤：...\n\n"
                + next_prompt
            )
...
def get_global_memory():
    ...
    try:
        ...
        with open(
            os.path.join(script_dir, "memory/global_mem_insight.txt"), "r", encoding="utf-8", errors="replace"
        ) as f:
            insight = f.read()
