...
def reload_mykeys():
    ...
    logger.info(f"[Info] Load mykeys from {_mykey_path}")
...
def compress_history_tags(messages, keep_recent=10, max_len=800, force=False, interval=5):
    ...
    _pats = {
        tag: re.compile(rf"(<{tag}>)([\s\S]*?)(</{tag}>)") for tag in ("thinking", "think", "tool_use", "tool_result")
    }
    ...
    logger.debug(f"[Cut] {_before} -> {sum(len(json.dumps(m, ensure_ascii=False)) for m in messages)}")
    return messages
...
_oldprint = print  # kept for backward compat
print = _oldprint  # restore original -- new code should use logging
...
def trim_messages_history(history, sess):
    ...
    logger.debug(f"[Debug] Current context: {cost()} chars, {len(history)} messages.")
    if cost() <= cap:
        return
    ...
    logger.debug(f"[Debug] Trimmed context, current: {cost()} chars, {len(history)} messages.")
...
def _parse_claude_sse(resp_lines):
    ...
    for line in resp_lines:
        ...
        try:
            ...
            logger.debug(f"[SSE] JSON parse error: {e}, line: {data_str[:200]}")
            continue
        ...
        if evt_type == "message_start":
            ...
            if out_tokens:
                logger.debug(f"[Output] tokens={out_tokens} stop_reason={stop_reason}")
    ...
    if current_block:
        ...
        current_block = None
    if warn:
        logger.warning(f"[WARN] {warn.strip()}")
...
def _record_usage(usage, api_mode):
    ...
    if api_mode == "responses":
        ...
        logger.debug(f"[Cache] input={inp} cached={cached}")
        ...
        logger.debug(f"[Cache] input={inp} creation={ci} read={cr}")
...
def _openai_stream(sess, messages):
    ...
    if api_mode == "responses":
        ...
        if sess.max_tokens:
            payload["max_completion_tokens" if ml.startswith(("gpt-5", "o1", "o2", "o3", "o4")) else "max_tokens"] = (
                sess.max_tokens
            )
        if sess.reasoning_effort:
...
class BaseSession:
    def __init__(self, cfg):
        ...
        def _enum(key, valid):
            ...
            return v if not v or v in valid else logger.warning(f"[WARN] Invalid {key} {v!r}, ignored.")
        ...
        if self.thinking_type:
            ...
            if self.thinking_type == "enabled":
                if self.thinking_budget_tokens is None:
                    logger.warning("[WARN] thinking_type='enabled' requires thinking_budget_tokens, ignored.")
                else:
    ...
    def ask(self, prompt):
        def _ask_gen():
            ...
            if len(content_blocks) > 1:
                logger.info(f"[DEBUG BaseSession.ask] content_blocks: {content_blocks}")
...
class ClaudeSession(BaseSession):
    def raw_ask(self, messages):
        ...
        parse_fn = (
            (lambda r: _parse_claude_sse(r.iter_lines())) if self.stream else (lambda r: _parse_claude_json(r.json()))
        )
    ...
    for m in messages:
        ...
        if fixed and fixed[-1]["role"] == "assistant" and m["role"] == "user":
            ...
            if miss:
                m = {
                    **m,
                    "content": [{"type": "tool_result", "tool_use_id": uid, "content": "(error)"} for uid in miss]
                    + _wrap(m["content"]),
                }
            orphan = has - set(uses)
            if orphan:
...
class NativeClaudeSession(BaseSession):
    ...
    def raw_ask(self, messages):
        ...
        payload["metadata"] = {
            "user_id": json.dumps(
                {"device_id": self._device_id, "account_uuid": self._account_uuid, "session_id": self._session_id},
                separators=(",", ":"),
            )
        }
        if self.tools:
            ...
            logger.error("[ERROR] No tools provided for this session.")
        payload["system"] = [
            {
                "type": "text",
...
class ToolClient:
    ...
    def chat(self, messages, tools=None):
        ...
        logger.info("Full prompt length:", len(full_prompt), "chars")
...
def _write_llm_log(label, content, log_path=None):
    if not log_path:
        log_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), f"temp/model_responses/model_responses_{os.getpid()}.txt"
        )
...
class MixinSession:
    ...
    def __init__(self, all_sessions, cfg):
        ...
        self._sessions = [
            ...
            if isinstance(i, int)
            else next(s.backend for s in all_sessions if type(s) is not dict and s.backend.name == i)
    ...
    def _raw_ask(self, *args, **kwargs):
        ...
        def test_error(x):
            return isinstance(x, str) and x.lstrip().startswith(("!!!Error:", "[Error:"))

        for attempt in range(self._retries + 1):
            idx = (base + attempt) % n
            gen = self._orig_raw_asks[idx](*args, **kwargs)
            logger.debug(f"[MixinSession] Using session ({self._sessions[idx].name})")
            ...
            if nxt == base:  # full round failed, delay before next
                ...
                logger.debug(f"[MixinSession] {last_chunk[:80]}, round {rnd} exhausted, retry in {delay:.1f}s")
                time.sleep(delay)
            else:
                logger.debug(f"[MixinSession] {last_chunk[:80]}, retry {attempt + 1}/{self._retries} (s{idx}→s{nxt})")
...
class NativeToolClient:
    ...
    def set_system(self, extra_system):
        ...
        if combined != self.backend.system:
            logger.debug(f"[Debug] Updated system prompt, length {len(combined)} chars.")
        ...
        for tr in tool_results:
            ...
            if tool_use_id:
                tool_result_blocks.append(
                    {"type": "tool_result", "tool_use_id": tool_use_id, "content": tr.get("content", "")}
                )
            else:
                combined_content = [
...
def resolve_session(cfg_name):
    ...
    if not cfg:
        raise ValueError(f"Config '{cfg_name}' not in mykey")
    if "native" in cfg_name:
        return (NativeClaudeSession if "claude" in cfg_name else NativeOAISession)(cfg=cfg)
...
def resolve_client(cfg_name):
    ...
    return (
        (NativeToolClient(s) if isinstance(s, (NativeClaudeSession, NativeOAISession)) else ToolClient(s))
        if s
        else None
    )
